...
- **카메라**: yaw 45°/pitch 55° 고정 쿼터뷰 + 직교 투영, MVP 행렬 기반 렌더링. `Camera::SetFocus()`로 플레이어를 따라다님. `Camera::SetFlip(horizontal, vertical)`로 화면을 좌우/상하 반전할 수 있고(투영 뒤 NDC x·y 부호만 뒤집으므로 카메라 방향·보이는 면·깊이 순서·컬링은 그대로, 그림만 뒤집힘) 현재는 둘 다 켜져 있음(그림이 180도 회전, `SimpleGame.cpp`의 `main()`에서 설정). HUD는 별도 화면 좌표계라 뒤집히지 않고, 이름표는 뒤집힌 화면 위치를 따라가며, WASD는 월드 축 기준이라 반전 후엔 화면상 방향이 반대로 보이므로 상하좌우(W/S, A/D) 모두 입력을 뒤집어 화면 방향에 맞춤(`Update()`의 `kInvertHorizontalInput`/`kInvertVerticalInput`) ([Camera.h](SimpleGame/Camera.h)/[.cpp](SimpleGame/Camera.cpp), [Math3D.h](SimpleGame/Math3D.h)). 이 월드는 Z축이 높이이므로 yaw는 `RotateZ`로 회전시킴 — 이전엔 `RotateY`를 써서 높이가 화면 가로 위치에 섞여 들어가고 Y축 이동이 대각선으로 투영되지 않는 비대칭 왜곡이 있었음
...
- **렌더링 최적화(유니폼/VAO 캐싱)**: 프로파일링([DrawCallHook.h](SimpleGame/DrawCallHook.h), 아래 "드로우 콜 카운터"/"프로파일러" 항목)으로 확인된 CPU 병목 두 가지를 고침. (1) 예전엔 `Draw*` 함수가 호출될 때마다 `glGetUniformLocation`/`glGetAttribLocation`을 매번 새로 조회했는데, 이제 `ShaderProgramInfo`/`PostProcess::PassShader` 구조체에 유니폼 위치를 셰이더 링크 직후 한 번만 캐시해 둠([Renderer.h](SimpleGame/Renderer.h)/[.cpp](SimpleGame/Renderer.cpp), [PostProcess.h](SimpleGame/PostProcess.h)/[.cpp](SimpleGame/PostProcess.cpp)). (2) VAO를 전혀 안 써서 매 드로우콜마다 `glEnableVertexAttribArray`/`glVertexAttribPointer`/`glDisableVertexAttribArray`를 반복했는데, 모든 정점 셰이더의 `a_Position`을 `layout(location=0)`으로 고정해 두고(`Shaders/*.vs`) VAO를 미리 만들어 재사용함 — 정적 사각형/동적 버퍼용 VAO는 `Renderer::Initialize()`에서 한 번(`m_VaoRect`/`m_VaoDynamic`), 메시별 VAO는 `CreateMesh()`가 만들 때(`MeshHandle::vao`), 후처리 풀스크린 사각형용 VAO는 `PostProcess::CreateFullscreenQuad()`에서 한 번만 만듦. 깊이테스트(`GL_DEPTH_TEST`)는 의도적으로 안 켬 — 이 씬은 그리기 순서(페인터 알고리즘)로 깊이를 흉내 내는 구조라, 실제 투영 Z(그리기 순서 정렬 기준인 `y+z`와 다른 값)로 깊이테스트를 켜면 캐릭터 파츠 등이 잘못 가려질 위험이 있어 시각적으로 검증하기 전엔 손대지 않기로 함
- **타일 배칭**: 32×32=1024칸 타일맵이 예전엔 타일 하나당 액터 하나·드로우콜 하나였는데(맵 전체가 보이면 그것만으로 드로우콜 1024회, 성능 분석에서 가장 큰 병목으로 확인됨), 이제 8×8칸 청크마다 정점 색상 메시 하나(`TileBatchActor`, [WorldActors.h](SimpleGame/WorldActors.h)/[.cpp](SimpleGame/WorldActors.cpp))로 구워서 드로우콜 1~2회(지형용 + 물이 있으면 물용)로 그림(`SpawnTileActors`, [LevelBuilder.cpp](SimpleGame/LevelBuilder.cpp)). 정점은 로컬이 아니라 월드 좌표로 미리 구워 둬서 오브젝트별 모델 행렬이 필요 없고(카메라 view-projection만 곱함), 뷰 컬링용 위치/경계 구만 청크 범위로 따로 지정함. 물결 애니메이션은 타일별 위상(`phase = x*1.7+y*2.3`)을 예전엔 CPU가 타일마다 계산해 유니폼으로 넘겼지만, 배칭 후엔 정점이 자기 타일의 월드 중심을 들고 있어서 셰이더 안에서 타일별로 다시 계산함(`Shaders/TileBatch.vs`+`TileBatchSolid.fs`/`TileBatchWater.fs`, 화면상 물결은 이전과 동일). 이 변경으로 `Renderer::DrawWater`/`Shaders/Water.vs`/`.fs`는 아무도 안 써서 삭제함
- **프로파일러**: [Profiler.h](SimpleGame/Profiler.h)/[.cpp](SimpleGame/Profiler.cpp)가 프레임을 구간(갱신/씬렌더.바닥/장식/그림자/오브젝트/후처리/HUD/이름표/미니맵/채팅창/화면출력)으로 나눠 CPU 시간을 재고, 1초마다(드로우 콜 카운터와 같은 간격이라 두 로그가 나란히 찍힘) 콘솔에 구간별 평균 ms와 프레임 대비 비중(%)을 한 줄로 출력함 — 이름을 코드가 실제로 하는 일 그대로 붙여서(예: "씬렌더.바닥") 사람이 훑어봐도, 나중에 이 로그를 그대로 AI에게 분석시켜도 바로 알아볼 수 있는 "AI 친화적" 형식을 의도함. 비중이 전체 프레임의 25%(`kHotspotFraction`) 이상인 구간엔 `[주의]` 표시가 붙어서, 지금 당장 아니어도 콘텐츠가 늘어나면 느려질 만한 지점을 미리 짚어 줌. 재는 법은 `Profiler::ScopedTimer`(생성자에서 시작 시각을 기록하고 소멸자에서 자동으로 끝남 — return으로 일찍 빠져나가도 안전) RAII 객체를 구간 블록에 두는 방식. `Idle()` 맨 앞에서 `BeginFrame()`, `RenderScene()` 끝에서 드로우 콜 카운터와 같은 note를 붙여 `EndFrame(note)`을 호출함
